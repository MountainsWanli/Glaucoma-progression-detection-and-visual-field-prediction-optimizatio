...
input_format = {0: 'CFPs2/', 1: 'ROI_images_3/', 2: 'Annotated Images/'}
format_index = 0  # format_index can be 0, 1, 2
images = []
...
for epoch in tqdm.tqdm(range(Epoch)):
    ...
    for data in train_loader:
        ...
        scheduler.step()
        if best_loss>loss:
            torch.save(model.state_dict(),'./checkpoints/resnet50Transformer_CFP/best.pt')
        best_loss=loss
...
model.load_state_dict(torch.load('./checkpoints/resnet50Transformer_ROI_images2/' + 'best.pt'))
model.eval()
test_pre = []
with torch.no_grad():
